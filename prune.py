...
def genetic_prune(args, model, tokenizer, dev, prune_n=0, prune_m=0):
    ...
    # a = - m1 / args.sparsity_ratio
    # b = (1.0 - args.sparsity_ratio) * m1 / args.sparsity_ratio
    ...
    for generation in range(num_generations):
        for _ in range(population_size):
            ...
            for (i, name), mask in candidate.items():
                # flip_prob = torch.rand_like(mask.float())
                # mask_1 = mask == 1
                # mask_0 = mask == 0

                # mask[mask_1 & (flip_prob < m1)] = 0
                # mask[mask_0 & (flip_prob < m2)] = 1
                layers = model.model.layers
