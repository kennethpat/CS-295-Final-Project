...
def check_sparsity(args, model):
    ...
    sparsity_ratio = float(count) / total_params
...
def genetic_prune(args, model, tokenizer, dev, prune_n=0, prune_m=0):
    ...
    population_size = 30
    num_generations = 100
    m2 = 0.005  # mutation rate for 0s
    m1 = (1.0 - args.sparsity_ratio) * m2 / args.sparsity_ratio  # mutation rate for 1s

    def getFirstIndividule(m):
        local_model = copy.deepcopy(m)
        args.prune_method = 'magnitude'
        prune_magnitude(args, local_model, tokenizer, dev, prune_n, prune_m)
...
def prune_magnitude_Monte_Carlo(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    while True:
        # Calculate the current sparsity ratio
        num_zeros = sum(torch.sum(p == 0) for p in model.parameters())
        total_params = sum(p.numel() for p in model.parameters())
        current_sparsity_ratio = num_zeros / total_params
...
def prune_magnitude_Monte_Carlo(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    while True:
        # Calculate the current sparsity ratio
        num_zeros = sum(torch.sum((p.to(device) == 0)) for p in model.parameters())
        total_params = sum(p.to(device).numel() for p in model.parameters())
        current_sparsity_ratio = num_zeros / total_params
        ...
        for i, layer in enumerate(layers):
            ...
            for name, module in subset.items():
                ...
                print(f"pruning layer {i} name {name}")
                W = module.weight.data.clone().to(device)
                num_weights = W.numel()
                num_prune = int(num_weights * (final_sparsity_ratio - current_sparsity_ratio))
                ...
                # Pruning
                pruning_threshold = W_metric.kthvalue(num_prune, dim=0, keepdim=True)[0]
                W_mask = (W.flatten() < pruning_threshold).view(W.shape).to(device)
                module.weight.data[W_mask] = 0
'''
...
def prune_magnitude_Monte_Carlo(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i, layer in enumerate(layers):
        ...
        for name, module in subset.items():
            if args.sparsity_type == "structured" and args.matrix != name:
                continue
            ...
            # Pruning
            pruning_threshold = W_metric.kthvalue(num_prune, dim=0, keepdim=True)[0]
            W_mask = (W.flatten() < pruning_threshold).view(W.shape).to(W.device)
            module.weight.data[W_mask] = 0
'''            
