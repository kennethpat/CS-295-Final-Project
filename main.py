...
def main():
    ...
    parser.add_argument("--sparsity_type", type=str, default="unstructured", help="Sparsity type, choose from unstructured, 4:8, 1:4, 2:4, 3:4. \
                        Please choose from the corresponding sparsity ratio")
    parser.add_argument("--prune_method", type=str, choices=["svd_finetuned", "magnitude", "ri", "wanda", "svd_ri", "svd", "sparsegpt", "ria", "gradient", "entropy", "bayesian", "structured"])
    parser.add_argument("--cache_dir", default="llm_weights", type=str )
    parser.add_argument('--save', action="store_true")
    parser.add_argument('--save_model', type=str, default=None, help='Path to save the pruned model.')
    parser.add_argument('--semi_sparse_acc', action="store_true", help="using pytorch semi sparse acceleration. Only when sparsity type is 2:4")
...
def main():
    ...
    if args.sparsity_ratio != 0:
        print("pruning starts")
        from lib.prune import prune_magnitude, gradient_pruning, entropy_pruning, prune_magnitude_Monte_Carlo, prune_ria, prune_sparsegpt, prune_magnitude_layer, gradient_pruning_layer, entropy_pruning_layer, check_sparsity, prune_magnitude_Monte_Carlo_structured, gradient_pruning_structured, entropy_pruning_structured

        # Compute gradients
        dummy_input = torch.randn(1, args.seqlen, dtype=torch.float32, device=device)
